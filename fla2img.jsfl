...
function extractImages() {
	...
	alert(
		'Getting ready to extract images from ' + documents.length + ' file' + (documents.length > 1 ? 's' : '') + '...'
	);

	// Creating an empty document to perform all the actions.
	temp = new fl.createDocument();

	var document, path, image, name, imagesFolder;
	...
	for (var count = 0; count < documents.length; ++count) {
		...
		const docpath = document.path.replace('\\', '/');

		// The path is relative to .fla file.
		path = 'file:///' + docpath.slice(0, docpath.lastIndexOf('/'));
